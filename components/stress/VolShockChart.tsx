...
import { volShocks } from "@/lib/data/stress";
...
function CustomTooltip({ active, payload }: any) {
  ...
  const pts = d.volShock * 100;
  return (
    <div style={tooltipStyle}>
      <p className="font-mono text-text-primary">
        Vol shock {pts > 0 ? "+" : ""}{pts.toFixed(1)} vol pts
      </p>
      <p className="mt-1 text-text-muted">Scenario vol: {formatPercent(d.scenarioVol, 1)}</p>
...
export function VolShockChart({ height = 196 }: { height?: number }) {
  const data = volShocks.map((s) => ({ ...s, shockPts: s.volShock * 100 }));
  return (
    <ResponsiveContainer width="100%" height={height}>
      <LineChart data={data} margin={{ top: 10, right: 16, bottom: 18, left: 6 }}>
        ...
        <XAxis
          ...
          label={{ value: "Volatility shock (vol points)", position: "insideBottom", offset: -8, fontSize: 11, fill: COLORS.textMuted }}
        />
        <YAxis
